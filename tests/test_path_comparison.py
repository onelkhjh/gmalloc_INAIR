...
def test_path_comparison_uses_one_fixed_assignment_and_separates_distance_models() -> None:
    ...
    assert [node.cell_count for node in report.current_approx_metric_tsp.nodes] == [23, 38, 19, 29]
    assert [node.cell_count for node in report.public_code_nn.nodes] == [23, 38, 19, 29]
    assert report.current_approx_metric_tsp.direct_makespan_m < report.public_code_nn.direct_makespan_m
    assert report.current_approx_metric_tsp.direct_total_m < report.public_code_nn.direct_total_m
    assert all(node.transit_inflation >= 1.0 for node in report.current_approx_metric_tsp.nodes)
